...
xplBase(exp,extension) = { 
  ...
  key               -> ('EOF' | 'thunk' | 'not' | 'and' | 'or' | 'when' | 'case' | 'module' | 'export' | 'import' | 'true' | 'false' | 'fun' | 'let' | 'in' | 'letrec' | 'if' | 'then' | 'else' | 'SKIPWHITE'| 'PARSE' | 'REGEXP')!;
  ...
  pAtom             -> endOfInput | cut | pred | charCode | pDelay | lDelay | notTerm | call | eCall | term | range | dot | action | skip | parse | regexp | pushLine | '(' a=pTerm ')' {a};
  cut               -> '!' {'grammar.Cut'()};
  ...
  parse             -> 'PARSE' '(' name=string ')' { 'grammar.Parse'(name) };
  regexp            -> 'REGEXP' '(' name=string ')' { 'grammar.Regexp'(name) };
