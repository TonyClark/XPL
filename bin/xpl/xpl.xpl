...
xplBase(exp,extension) = { 
  ...
  key               -> ('EOF' | 'thunk' | 'not' | 'and' | 'or' | 'when' | 'case' | 'module' | 'export' | 'import' | 'true' | 'false' | 'fun' | 'let' | 'in' | 'letrec' | 'if' | 'then' | 'else' | 'SKIPWHITE'| 'PARSE')!;
  ...
  pAtom             -> endOfInput | cut | pred | charCode | pDelay | lDelay | notTerm | call | eCall | term | range | dot | action | skip | parse | pushLine | '(' a=pTerm ')' {a};
  cut               -> '!' {'grammar.Cut'()};
